...
import { FinalStageService, prettyRound, roundSortKey } from '../../fixtures/final-stage.service';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatchRowComponent, FinalBracketComponent, EmptyStateComponent, LoadingSpinnerComponent],
      ...
      @if (isWorking()) {
        ...
        @if (!groupStageComplete()) {
          <app-empty-state
            ...
            [subtitle]="'Enter every group-stage result first — ' + progress().played + ' / ' + progress().total + ' played.'"
          />
        } @else if (auth.isAdmin() && !locked()) {
          <p class="text-sm text-gray-500">
            The group stage is complete. Choose how many teams advance from each group, then seed the bracket.
          </p>
          <label class="flex items-center gap-2 text-sm">
            <span class="text-gray-600 font-medium">Teams advancing per group</span>
            <select
          ...
          <p class="text-xs text-gray-400">{{ qualifiers() * 2 }} teams in the knockout — {{ bracketShape() }}.</p>
          <button class="btn-primary self-start flex items-center gap-1 !py-2 !px-4 text-sm" (click)="generate()">
            <span class="material-icons text-[18px]">account_tree</span> Generate Final Stage
          </button>
        } @else {
          <app-empty-state icon="account_tree" title="Final stage not set up yet" subtitle="Check back once the organiser seeds the bracket." />
        }
      } @else {
        @if (auth.isAdmin() && !locked()) {
          <div class="flex flex-col gap-1">
            <button class="btn-primary self-start flex items-center gap-1 !py-2 !px-4 text-sm" (click)="generate()">
              <span class="material-icons text-[18px]">account_tree</span> Generate / update bracket
            </button>
            <span class="text-xs text-gray-400">
              Winners &amp; losers drop into the next round automatically — use this if they didn't.
            </span>
          </div>
        }
        ...
        <div class="flex items-center justify-between gap-2">
            ...
            @if (view() === 'list') {
              @for (f of filterChips(); track f.value) {
                <button
                  ...
                  [class]="activeFilter() === f.value ? 'bg-primary-500 text-white' : 'bg-white border border-gray-200 text-gray-600'"
          ...
          <div class="flex rounded-lg bg-gray-100 p-1 text-xs font-semibold shrink-0">
            @for (v of ['chart', 'list']; track v) {
              <button
                class="px-3 py-1 rounded-md capitalize"
                [class]="view() === v ? 'bg-white shadow-sm text-primary-700' : 'text-gray-500'"
        ...
        @if (view() === 'chart') {
          <app-final-bracket [matches]="knockout()" [managers]="managerByTeam()" />
        } @else {
          @for (mt of visibleMatches(); track mt.id) {
            <div class="flex flex-col gap-2">
              <h3 class="text-sm font-extrabold uppercase tracking-wide text-gray-700">{{ pretty(mt.round) }}</h3>
              <app-match-row
                [match]="mt"
                [managers]="managerByTeam()"
                [showResultLink]="auth.isAdmin() && mt.status !== 'completed' && !!mt.homeTeamId && !!mt.awayTeamId"
              />
            </div>
          }
        }
...
export class FinalStageComponent {
  ...
  private matches = toSignal(
    toObservable(this.tournamentId).pipe(switchMap((id) => this.matchService.streamByTournament(id))),
  ...
  groupStageComplete = computed(() => this.finalStage.isGroupStageComplete(this.matches()));
  progress = computed(() => this.finalStage.groupStageProgress(this.matches()));

  qualifierOptions = computed(() => {
    const max = Math.min(this.finalStage.maxQualifiersPerGroup(this.matches()), 8);
  ...
  knockout = computed(() =>
    [...this.matches().filter((m) => !m.groupName)].sort((a, b) => roundSortKey(a.round) - roundSortKey(b.round))
  );
  ...
  filterChips = computed(() => {
    ...
    return [{ label: 'All', value: 'all' }, ...bases.map((b) => ({ label: prettyRound(b), value: b }))];
  });

  visibleMatches = computed(() => {
    const filter = this.activeFilter();
    return this.knockout().filter((m) => filter === 'all' || roundBase(m.round) === filter);
  });
  ...
  async generate(): Promise<void> {
    ...
    try {
      ...
      this.errorMsg.set(e instanceof Error ? e.message : 'Could not generate the final stage.');
    } finally {
